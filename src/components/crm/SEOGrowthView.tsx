...
import { SEOAdminPlatformSuite } from '../seo/SEOAdminPlatformSuite';
import { PublicSEOLandingView } from '../seo/PublicSEOLandingView';

export const SEOGrowthView: React.FC = () => {
  // Main view mode: 'admin_suite' (Admin SEO Control Panel) | 'public_landing' (Public SEO Pages) | 'legacy_rankings' (Quick Tracker)
  const [viewMode, setViewMode] = useState<'admin_suite' | 'public_landing' | 'legacy_rankings'>('admin_suite');
  
  // Legacy quick tabs
  const [activeTab, setActiveTab] = useState<'keywords' | 'audit' | 'backlinks' | 'meta_optimizer'>('keywords');
  const [keywords] = useState<SEOKeyword[]>(INITIAL_SEO_KEYWORDS);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Architecture Mode Switcher Bar */}
      <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <div className="text-xs font-bold text-white flex items-center gap-2">
              <span>Education Platform SEO Architecture</span>
              <span className="px-2 py-0.2 rounded bg-indigo-500/20 text-indigo-300 font-mono text-[10px] border border-indigo-500/30">
                RBAC Backend + Public Frontend
              </span>
            </div>
            <div className="text-[11px] text-slate-400">
              Universities, Colleges, Courses, Exams, Coaching, Schools, Sitemaps, Robots.txt &amp; AI CMS
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            onClick={() => setViewMode('public_landing')}
            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition flex items-center space-x-1.5 ${
          ...
          >
            ...
            <span>Public SEO Portal</span>
          </button>

          <button
            onClick={() => setViewMode('legacy_rankings')}
            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition flex items-center space-x-1.5 ${
              viewMode === 'legacy_rankings'
                ? 'bg-cyan-600 text-white shadow-lg shadow-cyan-950'
                : 'bg-slate-950 text-slate-400 hover:text-white border border-slate-800'
            }`}
          >
            <TrendingUp className="w-3.5 h-3.5" />
            <span>Quick Rank Tracker</span>
          </button>
        </div>
      </div>

      {/* Render Mode 1: Admin SEO Platform Suite */}
      {viewMode === 'admin_suite' && (
        <SEOAdminPlatformSuite 
          onNavigateToPublicLanding={() => setViewMode('public_landing')}
        />
      )}

      {/* Render Mode 2: Public SEO Landing Pages Simulation */}
      {viewMode === 'public_landing' && (
        <PublicSEOLandingView 
          onNavigateToAdminSEO={() => setViewMode('admin_suite')}
        />
      )}

      {/* Render Mode 3: Quick Rank Tracker & Site Health */}
      {viewMode === 'legacy_rankings' && (
        <div className="space-y-6">
              ...
              <div className="space-y-1">
                <div className="flex items-center space-x-2">
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center gap-1.5">
                    ...
                    Quick Search Audit &amp; Rankings Overview
                  </span>
